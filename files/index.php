...
class ImageServer
{
	...
	public static function showImage()
	{
		...
		{
			if(strlen($_GET['thumb']) > 0 && FTP::getConfig('thumbnails') == true)
			{
	...
	{
		if(is_int(FTP::getConfig('thumbnails_width')))
			$max_width = FTP::getConfig('thumbnails_width');
		else
			$max_width = 200;
		
		if(is_int(FTP::getConfig('thumbnails_height')))
			$max_height = FTP::getConfig('thumbnails_height');
		else
			$max_height = 200;
...
class Logger
{
	...
	{
		global $FTP;
		if(strlen(FTP::getConfig('log_file')) > 0)
		{
			if(Location::isFileWritable(FTP::getConfig('log_file')))
			{
				$message = "[" . date("Y-m-d h:i:s", mktime()) . "] ".$message." (".$_SERVER["HTTP_USER_AGENT"].")\n";
				error_log($message, 3, FTP::getConfig('log_file'));
			}
			else
				$FTP->setErrorString("log_file_permission_error");
		}
	}
	...
	{
		if(strlen(FTP::getConfig('upload_email')) > 0)
		{
			...
			mail(FTP::getConfig('upload_email'), "Upload notification", $message);
		}
	}
}
...
class GateKeeper
{
	...
	{
		global $FTP;
		if(strlen(FTP::getConfig("session_name")) > 0)
				session_name(FTP::getConfig("session_name"));
				
		if(count(FTP::getConfig("users")) > 0)
			session_start();			
		else
			return;
		...
		if(isset($_POST['user_pass']) && strlen($_POST['user_pass']) > 0)
		{
			if(GateKeeper::isUser((isset($_POST['user_name'])?$_POST['user_name']:""), $_POST['user_pass']))
			{
				...
				$FTP->setErrorString("wrong_pass");
		}
	}
	
	public static function isUser($userName, $userPass)
	{
		foreach(FTP::getConfig("users") as $user)
		{
			if($user[1] == $userPass)
			{
	...
	{
		if(FTP::getConfig("require_login") == false){
			return false;
		}
		return true;
	}
	...
	public static function isUploadAllowed(){
		if(FTP::getConfig("upload_enable") == true && GateKeeper::isUserLoggedIn() == true && GateKeeper::getUserStatus() == "admin")
			return true;
		return false;
	}
	
	public static function isNewdirAllowed(){
		if(FTP::getConfig("newdir_enable") == true && GateKeeper::isUserLoggedIn() == true && GateKeeper::getUserStatus() == "admin")
			return true;
		return false;
	}
	
	public static function isDeleteAllowed(){
		if(FTP::getConfig("delete_enable") == true && GateKeeper::isUserLoggedIn() == true && GateKeeper::getUserStatus() == "admin")
			return true;
		return false;
	}
	
	public static function getUserStatus(){
		if(GateKeeper::isUserLoggedIn() == true && FTP::getConfig("users") != null && is_array(FTP::getConfig("users"))){
			foreach(FTP::getConfig("users") as $user){
	...
	public static function showLoginBox(){
		if(!GateKeeper::isUserLoggedIn() && count(FTP::getConfig("users")) > 0)
			return true;
		return false;
	}
}
...
class FileManager
{
	...
	{
		global $FTP;
		if(strlen(FTP::getConfig("upload_password")) > 0 && $inputPassword == FTP::getConfig("upload_password"))
		{
			return true;
		}
		else
		{
			$FTP->setErrorString("wrong_password");
			return false;
		}
	}
	*/
	...
	function uploadFile($location, $userfile)
	{
		...
		{
			$FTP->setErrorString("failed_upload");
		}
		else if(is_array(FTP::getConfig("upload_allow_type")) && count(FTP::getConfig("upload_allow_type")) > 0 && !in_array($mime_type, FTP::getConfig("upload_allow_type")))
		{
			$FTP->setErrorString("upload_type_not_allowed");
		}
		else if(is_array(FTP::getConfig("upload_reject_extension")) && count(FTP::getConfig("upload_reject_extension")) > 0 && in_array($extension, FTP::getConfig("upload_reject_extension")))
		{
			$FTP->setErrorString("upload_type_not_allowed");
		}
		else if(!@move_uploaded_file($userfile['tmp_name'], $upload_file))
		{
			$FTP->setErrorString("failed_move");
		}
		else
		{
...
class File
{
	...
	public static function getFileSize($file)
	{
		...
		if (FTP::getConfig("large_files") == true || !$sizeInBytes || $sizeInBytes < 0) {
...
class Location
{
	...
	{
		return (strlen(FTP::getConfig('basedir')) > 0?FTP::getConfig('basedir'):dirname($_SERVER['SCRIPT_FILENAME']))."/".$this->getDir(true, false, false, 0);
	}
	...
	function init()
	{
		...
		{
			$this->path = $this->splitPath(FTP::getConfig('starting_dir'));
		}
		else
		{
	...
	{
		if(FTP::getConfig('upload_enable') != true)
			return false;
		if(FTP::getConfig('upload_dirs') == null || count(FTP::getConfig('upload_dirs')) == 0)
			return true;
			
		$upload_dirs = FTP::getConfig('upload_dirs');
...
class FTP
{
	...
	function init()
	{
		...
		if(isset($_GET['lang']) && isset($_TRANSLATIONS[$_GET['lang']]))
			...
			$this->lang = FTP::getConfig("lang");
		
		$this->mobile = false;
		if(FTP::getConfig("mobile_enabled") == true)
		{
			if((FTP::getConfig("mobile_default") == true || isset($_GET['m'])) && !isset($_GET['s']))
				$this->mobile = true;
		}
		
		$this->logging = false;
		if(FTP::getConfig("log_file") != null && strlen(FTP::getConfig("log_file")) > 0)
			$this->logging = true;
	}
	...
	function readDir()
	{
		...
		if($open_dir = @opendir($this->location->getFullPath()))
		{
			...
			while ($object = readdir($open_dir))
			{
				if($object != "." && $object != "..") 
				{
					...
					{
						if(!in_array($object, FTP::getConfig('hidden_dirs')))
							$this->dirs[] = new Dir($object, $this->location);
					}
					else if(!in_array($object, FTP::getConfig('hidden_files')))
		...
		{
			$FTP->setErrorString("unable_to_read_dir");;
		}
	}
	...
	{
		if(FTP::getConfig('calculate_space_level') <= 0)
			return;
		$ignore_files = array('..', '.');
		$start_dir = getcwd();
		$spaceUsed = $this->sum_dir($start_dir, $ignore_files, FTP::getConfig('calculate_space_level'));
	...
	function sort()
	{
		if(is_array($this->files)){
			usort($this->files, "FTP::cmp_".$this->sort_by);
			if($this->sort_as == "desc")
				$this->files = array_reverse($this->files);
		}
		
		if(is_array($this->dirs)){
			usort($this->dirs, "FTP::cmp_name");
	...
	{
		$link = "?";
		if($switchVersion == true && FTP::getConfig("mobile_enabled") == true)
		{
		...
		else if($this->mobile == true && FTP::getConfig("mobile_enabled") == true && FTP::getConfig("mobile_default") == false)
			$link .= "m&amp;";
		else if($this->mobile == false && FTP::getConfig("mobile_enabled") == true && FTP::getConfig("mobile_default") == true)
			$link .= "s&amp;";
		...
		if(isset($this->lang) && $this->lang != FTP::getConfig("lang"))
			$link .= "lang=".$this->lang."&amp;";
	...
	function formatModTime($time)
	{
		...
		if(FTP::getConfig("time_format") != null && strlen(FTP::getConfig("time_format")) > 0)
			$timeformat = FTP::getConfig("time_format");
	...
	{
		return FTP::getLangString($stringName, $this->lang);
	}
	...
	{
		FTP::setError($this->getString($stringName));
	}
...
<?php 
	...
	if(FTP::getConfig("thumbnails") == true && $this->mobile == false)
	{
?>
...
?>
<title><?php if(FTP::getConfig('main_title') != null) print FTP::getConfig('main_title'); ?></title>
</head>
...
<div id="frame">
<?php
if(FTP::getConfig('show_top') == true)
{
?>
<div id="top">
	<a href="<?php print $this->makeLink(false, false, null, null, null, ""); ?>"><span><?php if(FTP::getConfig('main_title') != null) print FTP::getConfig('main_title'); ?></span></a>
<?php 
if(FTP::getConfig("secondary_titles") != null && is_array(FTP::getConfig("secondary_titles")) && count(FTP::getConfig("secondary_titles")) > 0 && $this->mobile == false)
{
	$secondary_titles = FTP::getConfig("secondary_titles");
...
{
if($this->mobile == false && FTP::getConfig("show_path") == true)
{
?>
...
<tr class="row one header">
	...
	<td class="del"><?php print FTP::getString("del"); ?></td>
	<?php } ?>
</tr>
<?php 
}
